  ...
  <section class="register">
    ...
    <el-form class="register-form" auto-complete="off" :model="model" :rules="rules" ref="registers" label-position="left">
      ...
      <el-form-item prop="type">
        <el-button-group>
          <el-button type="primary" @click="toEmail">
            {{$t('auth.toEmail')}}
          </el-button>
          <el-button type="info" @click="toPhone">
            {{$t('auth.toPhone')}}
          </el-button>
        </el-button-group>
      ...
      <div class="captcha-container" v-if="captcha_image_content_ed">
        <el-form-item prop="captcha_code">
            <el-row>
              ...
              <el-col :span="8">
                <img  :title="$t('auth.captchaTitle')" v-bind:src="captcha_image_content_ed" @click="get_captcha()"/>
              </el-col>
            </el-row>
        </el-form-item>
      </div>
    ...
    <el-dialog :title="$t('auth.validatePhone')" :visible.sync="dialogRegVisible" :before-close="handleClose">
...
export default {
  ...
  data () {
    ...
    return { model: model, rules: rules, error: null, loading: false,
      canSend:true,regCodeInfo:this.$t('auth.send'),intervalId:0,count:60,
  ...
  methods: {
    ...
    send(done){
      ...
      var self = this;
      axios.post('verificationCodes/phone', params).then(function(res){
    ...
    submit(){
      this.$refs.registers.validate(valid => {
        ...
        if(this.captcha_image_content){
          //get the code
          /*let params = {
            captcha_code:self.model.captcha_code,
            captcha_key:self.model.captcha_key
          };
          axios.post('verificationCodes/phone', params).then(function(res){
            self.model.verification_key = res.data.key || "";
    ...
    get_captcha(){
      self = this;
      let params = {type:'phone', phone: this.model.phone}
